...
import { ChooseTeam } from "./ChooseTeam"; // Make sure this path is correct!
...
describe("ChooseTeam Component tests", () => {
    ...
    test("Line 19: Clicking the same member twice does not add them again", () => {
        // No need for render(<ChooseTeam />) here, beforeEach handles it!

        // 1. Find the button for a specific person
        const addButton = screen.getByRole("button", { name: /Alan Turing/i });

        // 2. Click once (Hits the 'false' branch: adding to the array)
        fireEvent.click(addButton);

        // 3. Click again (Hits the 'true' branch: Line 19 - returning 'prev')
        fireEvent.click(addButton);

        // 4. Verify they only appear once in the Team list
        const listItems = screen.getAllByRole("listitem");
        const turingEntries = listItems.filter(
            (item) => item.textContent === "Alan Turing",
        );

        expect(turingEntries.length).toBe(1);
    });
});
